...
<!-- TOC -->
- [Android SDK集成指南](#android-sdk集成指南)
- [目的](#目的)
- [前提条件](#前提条件)
- [开发环境](#开发环境)
- [硬件设备](#硬件设备)
...
- [Demo程序](#demo程序)
  - [demo 架构介绍](#demo-架构介绍)
- [1.下载Android SDK](#1下载android-sdk)
- [2.Android Studio环境搭建](#2android-studio环境搭建)
- [3.AndroidManifest.xml 设置](#3androidmanifestxml-设置)
- [4.SDK 初始化](#4sdk-初始化)
- [5.SDK 使用流程](#5sdk-使用流程)
- [6.用户系统](#6用户系统)
  - [(1) SFUser类](#1-sfuser类)
  - [(2) 用户登录](#2-用户登录)
    - [1、通过extra注册或登录（首次调用为注册）](#1通过extra注册或登录首次调用为注册)
    - [2、通过loginKey登录](#2通过loginkey登录)
  - [(3) 获取缓存用户](#3-获取缓存用户)
- [7.路由器系统](#7路由器系统)
  - [(1) Routers类](#1-routers类)
  - [(2) 绑定路由器](#2-绑定路由器)
  - [(3) 获取用户绑定的路由器](#3-获取用户绑定的路由器)
- [8. WebSocket连接](#8-websocket连接)
- [9.远程接口](#9远程接口)
  - [(1)远程接口调用流程](#1远程接口调用流程)
  - [(2)调用API示例](#2调用api示例)
- [10.局域网接口](#10局域网接口)
  - [(1) 局域网接口调用流程](#1-局域网接口调用流程)
  - [(2) LocalApi 初始化](#2-localapi-初始化)
  - [(3) API调用示例](#3-api调用示例)
- [测试](#测试)
  - [1 网络状态测试](#1-网络状态测试)
    - [(1) 手机与siflower服务连接测试](#1-手机与siflower服务连接测试)
    - [(2) 手机与局域网路由器连接测试](#2-手机与局域网路由器连接测试)
    - [(3) 路由器与siflower服务连接测试](#3-路由器与siflower服务连接测试)
  - [2 局域网接口测试](#2-局域网接口测试)
    - [(1) sdk接口测试](#1-sdk接口测试)
    - [(2) 路由器接口测试](#2-路由器接口测试)
  - [3 远程接口测试](#3-远程接口测试)
- [FAQ](#faq)
  - [1 用户登录失败](#1-用户登录失败)
  - [2 路由器绑定失败](#2-路由器绑定失败)
  - [3 websocket建立连接失败](#3-websocket建立连接失败)
  - [4 局域网接口调用失败](#4-局域网接口调用失败)
  - [5 远程接口调用失败](#5-远程接口调用失败)
...
![figure1](/assets/images/androidsdk/create_project.png          )-->
...
![libs](/assets/images/androidsdk/copy_to_lib.png)
